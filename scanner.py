...
UNIVERSE_FILE = "niftymidcap100_symbols.json"
...
def save_midcap100_symbols(symbols):
    with open(UNIVERSE_FILE, "w") as f:
        json.dump(symbols, f)


def load_midcap100_symbols(force_refresh=False):
    if force_refresh:
        symbols = fetch_midcap100_symbols()
        save_midcap100_symbols(symbols)
...
def run():
    ...
    nifty_midcap100_symbols = load_midcap100_symbols()
    state = load_state()
    signals = []
...
if __name__ == "__main__":
    ...
    if args.refresh_universe:
        load_midcap100_symbols(force_refresh=True)
    else:
        run()
